...
export default class Buho_lienholderInformation extends LightningElement {
    ...
    get lineHolderName() {
        return this.lienholderData.Lienholder_name__c ? this.lienholderData.Lienholder_name__c : '';
    }

    get postalCode() {
        return this.lienholderData.Lienholder_Postal_Code__c ? this.lienholderData.Lienholder_Postal_Code__c : '';
    }

    get lineHolderState() {
        return this.lienholderData.Lienholder_State__c ? this.lienholderData.Lienholder_State__c : '';
    }

    get lineHolderCity() {
        return this.lienholderData.Lienholder_City__c ? this.lienholderData.Lienholder_City__c : '';
    }

    get lineHolderStreet() {
        return this.lienholderData.Lienholder_Street__c ? this.lienholderData.Lienholder_Street__c : '';
    }

    get lineHolderPhone() {
        return this.lienholderData.Lienholder_Phone__c ? this.lienholderData.Lienholder_Phone__c : '';
    }
    ...
    // Handle regular input changes from buho_input
    handleInputChange(event) {
        const { name, value } = event.detail;
        let processedValue = value;

        if (/^\s/.test(processedValue)) {
            processedValue = '';
        }

        this.lienholderData = { ...this.lienholderData, [name]: processedValue };
    ...
    handlePhoneChange(event) {
        ...
        if (processedValue && processedValue.length > 0) {
            const x = processedValue.replace(/\D+/g, '').match(/(\d{0,3})(\d{0,3})(\d{0,4})/);
            console.log('--phone value--', x);
            let phonevalues = !x[2] ? x[1] : `(${x[1]}) ${x[2]}` + (x[3] ? `-${x[3]}` : ``);
            console.log('--phone value--', phonevalues);
            processedValue = phonevalues;
        }
    ...
    @api validate() {
        ...
        // Check other country input if "Other" is selected
        if (this.otherCountry && (!this.lienholderData.Lienholder_Country__c || this.lienholderData.Lienholder_Country__c.trim() === '')) {
            this.dispatchEvent(new CustomEvent('toastevent', {
                detail: {
                    variant: 'error',
                    title: 'Validation Error',
        ...
        // Check name
        if (!this.lienholderData.Lienholder_name__c || this.lienholderData.Lienholder_name__c.trim() === '') {
            this.dispatchEvent(new CustomEvent('toastevent', {
                detail: {
                    variant: 'error',
                    title: 'Validation Error',
            ...
            if (!this.lienholderData[field] || this.lienholderData[field].toString().trim() === '') {
                this.dispatchEvent(new CustomEvent('toastevent', {
                    detail: {
                        variant: 'error',
                        title: 'Validation Error',
                        message: `Please enter ${requiredFields[field]}.`
                    },
                    bubbles: true,
                    composed: true
                }));
