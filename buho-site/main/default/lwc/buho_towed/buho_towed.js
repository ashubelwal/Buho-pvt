...
const ICON_DELETE = BUHO_ASSETS + '/images/icon-delete.svg';
const ICON_INFO = BUHO_ASSETS + '/images/icon-info.svg';

export default class Buho_towed extends LightningElement {
    @api towedUnits = [];
    @api currentUserType;
    ...
    renderedCallback() {
        // Set select element values programmatically
        const towedTypeSelect = this.template.querySelector('select[name="Towed_Unit_Type__c"]');
        if (towedTypeSelect && this.formType) {
            towedTypeSelect.value = this.formType;
        }

        const existingTowSelect = this.template.querySelector('select[name="existingTow"]');
        if (existingTowSelect && this.selectedTowedId) {
            existingTowSelect.value = this.selectedTowedId;
        }
    }
    ...
    changetowedOption(event) {
        ...
        if (selectedVehicle) {
            ...
            this.validateForm();
            // Reset the dropdown after loading data
            this.selectedTowedId = '';
        }
    }
}