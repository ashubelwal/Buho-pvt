...
export default class Nc_finalDetails extends LightningElement {
    ...
    // Modal states
    showTermsOfPurchaseModal = false;
    showTermsOfCancellationModal = false;
    
    // Additional checkbox states
    fishingLicense = false;
    fmmTouristPermit = false;
    tipImportPermit = false;
    allThreeServices = false;
    standardProtection = false;
    vacationRentalRange = false;
    petCare = false;
    fmm = false;
    fishingLicenseDoc = false;
    tip = false;
    ...
    // Getters for readonly fields from payload
    get termDateRange() {
        const termOption = this.dataMap?.termOption;
        return termOption?.DateRange || '';
    }

    get coverageValue() {
        const quoteData = this.dataMap?.quotePage?.QuoteData;
        return quoteData?.Coverage__c || '';
    }

    get territoryValue() {
        const territory = this.dataMap?.territory;
        return territory?.region || '';
    }

    get companyValue() {
        const quoteData = this.dataMap?.quotePage?.QuoteData;
        return quoteData?.Underwriter__c || '';
    }

    get premiumAndFeesValue() {
        const quoteValue = this.dataMap?.quotePage?.QuoteData?.Quote_Value__c;
    
        return quoteValue !== null && quoteValue !== undefined
            ? `$${Number(quoteValue).toFixed(2)}`
            : '';
    }
    

    get policyNumberValue() {
        const quoteData = this.dataMap?.quotePage?.QuoteData;
        return quoteData?.Policy_Number__c || '';
    }

    // Modal handlers
    handleTermsOfPurchaseClick(event) {
        event.preventDefault();
        event.stopPropagation();
        this.showTermsOfPurchaseModal = true;
    }

    closeTermsOfPurchaseModal() {
        this.showTermsOfPurchaseModal = false;
    }

    handleTermsOfCancellationClick(event) {
        event.preventDefault();
        event.stopPropagation();
        this.showTermsOfCancellationModal = true;
    }

    closeTermsOfCancellationModal() {
        this.showTermsOfCancellationModal = false;
    }

    handleModalOverlayClick(event) {
        // Close modal if clicking on the overlay (not the content)
        if (event.target === event.currentTarget) {
            this.closeTermsOfPurchaseModal();
            this.closeTermsOfCancellationModal();
        }
    }
    ...
    handleContinue() {
        // Existing continue logic or call parent method
    ...
    handleChange(event) {
        ...
        if (name == 'Terms_of_Purchase_Confirmed__c' || name == 'Terms_of_Cancellation_Confirmed__c') {
            value = event.target?.checked || event.detail?.checked;
        ...
        if (name == 'Travel_Alerts__c' || name == 'Announcements__c' || name == 'Newsletter__c' || name == 'Yellow_Card__c' || 
            name == 'Fishing_License' || name == 'FMM_Tourist_Permit' || name == 'TIP_Import_Permit' || name == 'All_Three_Services' ||
            name == 'Standard_Protection' || name == 'Vacation_Rental_Range' || name == 'Pet_Care' ||
            name == 'FMM' || name == 'Fishing_License_Doc' || name == 'TIP') {
            value = event.target?.checked || event.detail?.checked;
            value = value ? 'Yes' : 'No';
        }
    ...
    fetchData = async () => {
        ...
        if (this.dataMap?.userDetails?.Id != undefined && !this.communityUser) {
            ...
            try {
                ...
                const data = await fetchDataFromLead({ "LeadId": this.dataMap?.quotePage?.QuoteData?.Lead__c, "screenName": "Terms_Alert__c" });
