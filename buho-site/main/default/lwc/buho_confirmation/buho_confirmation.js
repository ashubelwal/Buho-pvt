...
export default class Nc_confirmation extends NavigationMixin(LightningElement) {
     label = {
    ...
    get policyData() {
        return this.policyName?.data ?? ''
    }

    get policyEmail() {
        return this.policyName?.email ?? ''
    }
    
    async connectedCallback() {
        await this.initializeDataMap();
        this.policyName = this.dataMap?.payment;

        if (!this.policyName.PolicyId) {
            console.error('PolicyId missing. Aborting.');
            return;
        } 

        emailServices({ policyId: this.policyName.PolicyId, emailService: true })
        ...
        if( this.policyName ){
            if (!this.communityUser) {
                ...
                console.log('Window Data lAyer',window?.dataLayer);
                ...
                if (!eventExist){
                    ...
                    .then((result)=>{
                        if(result){
                            console.log('Result in the apex',result);
