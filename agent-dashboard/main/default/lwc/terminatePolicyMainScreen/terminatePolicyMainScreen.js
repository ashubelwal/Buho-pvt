...
export default class TerminatePolicyMainScreen extends NavigationMixin(LightningElement) {
    ...
    async connectedCallback() {
        ...
        if (this.recordId) {
            if(this.debug) console.log('recordId passed via API', this.recordId);
        } else if (params.get('c__id') !== null) {
            this.recordId = params.get('c__id');
        } else if (params.get('c__policyId') !== null) {
            this.recordId = params.get('c__policyId');
        } else {
            const sessionRecordId = sessionStorage.getItem('recordId');
            if(this.debug) console.log('Session Storage', sessionRecordId);
