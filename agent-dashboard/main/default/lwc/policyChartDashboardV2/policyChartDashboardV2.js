...
import defaultTemplate from './policyChartDashboardV2.html';
import newUiTemplate from './policyChartDashboardV2NewUI.html';
...
export default class PolicyChartDashboardV2 extends LightningElement {
    ...
    render() {
        return this.isNewUI ? newUiTemplate : defaultTemplate;
    }

    handlePolicyChange(event) {
        this.selectedPolicyType = event.detail ? event.detail.value : event.target.value;
        this.loadPolicyData();
    }

    createExcelWorksheet(name, headers, rows) {
        ...
        rows.forEach(r => {
            xml += `   <Row>\n`;
            r.forEach(c => {
                xml += `    <Cell><Data ss:Type="String">${c}</Data></Cell>\n`;
            });
            xml += `   </Row>\n`;
        });
        xml += `  </Table>\n </Worksheet>\n`;
    ...
    @api downloadCSV() {
        ...
        // 1. New Policies
        if (this.isPolicyForNew && this.chartData && this.chartData.length) {
            let policyTypes = [...new Set(this.chartData.map((policy) => policy.Policy_Type_picklist__c))];
            let rows = [];
            policyTypes.forEach(type => {
                const policy = this.chartData.find(p => p.Policy_Type_picklist__c === type && !p.renewed_check__c);
                if (policy && policy.totalCount > 0) {
                    rows.push([type, policy.totalCount]);
                }
            });
            if (rows.length > 0) {
                xmlStr += this.createExcelWorksheet('New Policies', ['Policy Type', 'Total Count'], rows);
                hasData = true;
            }
        }

        // 2. Renewed Policies
        if (this.isPolicyForRenew && this.chartData && this.chartData.length) {
            let policyTypes = [...new Set(this.chartData.map((policy) => policy.Policy_Type_picklist__c))];
            let rows = [];
            policyTypes.forEach(type => {
                const policy = this.chartData.find(p => p.Policy_Type_picklist__c === type && p.renewed_check__c);
            ...
            if (rows.length > 0) {
                xmlStr += this.createExcelWorksheet('Renewed Policies', ['Policy Type', 'Total Count'], rows);
                hasData = true;
            }
        }

        // 3. Agency Net Commission
        if (this.isAgency && this.netValueBarChartDataComission && this.netValueBarChartDataComission.length) {
            let rows = [];
            this.netValueBarChartDataComission.forEach(item => {
                rows.push([item.Policy_Type_picklist__c, item.totalCount]);
            });
            if (rows.length > 0) {
                xmlStr += this.createExcelWorksheet('Net Commission', ['Policy Type', 'Amount'], rows);
                hasData = true;
            }
        }

        // 4. Agency Net Premium
        if (this.isAgency && this.netValueBarChartData && this.netValueBarChartData.length) {
            let rows = [];
            this.netValueBarChartData.forEach(item => {
                rows.push([item.Policy_Type_picklist__c, item.totalCount]);
            });
            if (rows.length > 0) {
                xmlStr += this.createExcelWorksheet('Net Premium', ['Policy Type', 'Amount'], rows);
                hasData = true;
            }
        }

        if (!hasData) {
            xmlStr += this.createExcelWorksheet('No Data', ['Status'], [['No data available for the selected range']]);
        }

        xmlStr += `</Workbook>`;

        try {
            const encodedUri = 'data:application/vnd.ms-excel;base64,' + btoa(unescape(encodeURIComponent(xmlStr)));
            const link = document.createElement('a');
            link.href = encodedUri;
            link.setAttribute('download', 'Policy_Analytics_Data.xls');
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
        } catch (e) {
            if (this.debug) console.error('Error creating excel download link:', e);
        }
    }
        ...
        if (this.isAgency) {
            ...
            this.barChartValueData = new Chart(barCtx, {
                ...
                data: {
                    labels: barLabels,
                    datasets: [{
                        label: 'Policies Total Net', data: barData, backgroundColor: [
                            '#FF6384', // Pink-Red
                            '#36A2EB', // Blue
                            '#FFCE56', // Yellow
                            '#4BC0C0', // Teal
        ...
        if (this.isAgency) {
            ...
            this.barChartComissionData = new Chart(barCtx, {
                ...
                data: {
                    labels: barLabels,
                    datasets: [{
                        label: 'Policies Total Net Comission', data: barData, backgroundColor: [
                            '#FF6384', // Pink-Red
                            '#36A2EB', // Blue
                            '#FFCE56', // Yellow
                            '#4BC0C0', // Teal
