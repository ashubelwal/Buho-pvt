...
export default class PolicyEditRenew extends NavigationMixin(LightningElement) {
    ...
    handleStartTimeValidation(event) {
        ...
        if (
            ...
            if (converted < timedataconverted) {
            ...
            for (let eachParam in data) {
                if (eachParam) {
                    ...
                    if (templateElement != null) {
                        ...
                        let value = data[eachParam];

                        if (eachParam === "Start_Date_for_Coverage__c") {
                            const dataDate = new Date(value);
                            if (dataDate < today) {
                                value = today.toISOString().split('T')[0]; // "YYYY-MM-DD"
                            }
                            console.log('Inside date set');
                            this.trackVar.startDate = value;
                        }
        ...
        try {
            ...
            if (event.target.name === 'Medical__c' && this.actionmode == 'edit') {
                ...
                if ((parseInt(newIndex) - parseInt(oldIndex)) < 0) {
                    this.showToastEvent('Medical amount error', 'You cannot select Medical lower than $' + this.editOldProcessData.quoteData.Medical__c + '.', 'error');
                ...
                if (event.target.name === 'Liability__c' && this.actionmode == 'edit') {
                    ...
                    if ((parseInt(newIndex) - parseInt(oldIndex)) < 0) {
                        this.showToastEvent('Liability amount error', 'You cannot select Liability lower than $' + this.editOldProcessData.quoteData.Liability__c + '.', 'error');
    ...
    generatePolicyPDF(event) {
        ...
        validateandGenerateQuotePDF({ 'policyId': this.renewdPolicyId })
            .then((result) => {
                ...
                console.log(result);
                if (result) {
                    window.open(((this.isAgentPortal ? '/agencyvforcesite/' : '/') + 'apex/' + result + '?id=' + this.renewdPolicyId), '_blank');
                } else {
    ...
    async createCloneQuotes() {
        try {
            ...
            if (data.status == 'success') {
                ...
                this.showToastEvent('Unable to create Quote!', 'Something went wrong while creating quote.', 'error');
                return null;
            }
