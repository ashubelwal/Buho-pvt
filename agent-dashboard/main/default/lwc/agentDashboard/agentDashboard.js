...
import defaultTemplate from './agentDashboard.html';
import partnerTemplate from './partnerAgentDashboard.html';
...
export default class AgentDashboard extends NavigationMixin(LightningElement) {
    ...
    @track isAgentportal;
    timeZone = '';
    @track activeSections = ['lead'];
    @track activeSectionsMessage = '';

    @track expandLeads = true;
    @track expandContacts = true;
    @track expandPolicies = true;

    toggleLeads() { this.expandLeads = !this.expandLeads; }
    toggleContacts() { this.expandContacts = !this.expandContacts; }
    togglePolicies() { this.expandPolicies = !this.expandPolicies; }

    get searchContainerClass() {
        let hasData = this.booleanVar.leadDataStatus || this.booleanVar.contactDataStatus || this.booleanVar.policyDataStatus || this.booleanVar.noDataFound || this.booleanVar.createLeadButton;
        return hasData ? "section-card mb-4 w-100" : "section-card mb-4 w-50 mx-auto";
    }

    get leadsIconClass() { return this.expandLeads ? 'bi bi-chevron-up' : 'bi bi-chevron-down'; }
    get contactsIconClass() { return this.expandContacts ? 'bi bi-chevron-up' : 'bi bi-chevron-down'; }
    get policiesIconClass() { return this.expandPolicies ? 'bi bi-chevron-up' : 'bi bi-chevron-down'; }
    ...
    checkLocalStoreForSearch() {
        let storedSearch = localStorage.getItem('agentSearchData');
        if (storedSearch) {
            try {
                let cacheData = JSON.parse(storedSearch);
                let now = new Date().getTime();
                let fiveMinutes = 5 * 60 * 1000;

                if (now - cacheData.timestamp < fiveMinutes) {
                    if (cacheData.value && cacheData.value.agentSearchData) {
                        this.searchData = cacheData.value.agentSearchData;
                        if (cacheData.value.searchResults) {
                            this.searchResults = cacheData.value.searchResults;
                    ...
                    localStorage.removeItem('agentSearchData');
                }
            } catch (e) {
                localStorage.removeItem('agentSearchData');
            }
    ...
    setSearchResults(result) {
        ...
        // Store search value in local storage natively with 5 min expiry
        let cacheObj = {
            value: { 'agentSearchData': this.searchData, 'searchResults': this.searchResults },
            timestamp: new Date().getTime()
        };
        localStorage.setItem('agentSearchData', JSON.stringify(cacheObj));
    }
    ...
    handleRowAction(event) {
        console.log('handleRowAction - event', event.target.dataset);
        let actionName = event.detail?.action?.name || event.target.dataset.actiontype;
        let currentUrl = window.location.origin;
        const row = event.detail?.row?.Id || event.target.dataset.id;
        ...
        if (actionName === 'create_user') {
            ...
            if (this.isAgentPortal) {
                this.navigateInAgentPortal('agentQuickQuote', state);
            } else {
                if (this.cmpSource == 'comm') {
    ...
    handleLeadRowAction(event) {
        const actionName = event.detail?.action?.name || event.target.dataset.actiontype;
        let row = event.detail?.row?.Id || event.target.dataset.id;
        ...
        if (status) {
            ...
            if (this.searchData.searchValue.length > 1) {
                ...
                if (this.cmpSource === 'comm') {
                    ...
                    let endpointUrl;
                    await getCustomMetadataForApi({ 'developerName': 'Fetch_data_from_salesforce' })
                    .then((data) => {
...
.catch((err) => {
    ...
    handleNewLead = () => {
        ...
        if (this.isAgentPortal) {
            this.navigateInAgentPortal('agentQuickQuote', {});
        } else {
            let currentUrl = window.location.origin;
            if (this.cmpSource == 'comm') {
    ...
    navigateToPolicyRenew(event) {
        ...
        if (this.isAgentPortal) {
            this.navigateInAgentPortal('policyEditRenew', {
                'c__policyId': state.c__policyId,
                'c__actionmode': state.c__actionmode
            });
        } else {
            ...
            if (state.c__action == 'VIEWPOLICY') {
                ...
                validateandGenerateQuotePDF({ 'policyId': state.c__id })
                    .then((result) => {
                        ...
                        if (result) {
                            if(this.isAgentPortal) {
                                window.open(('/agencyvforcesite/apex/' + result + '?id=' + state.c__id), '_blank');
                            } else {
    ...
    handleOperations = (event) => {
        ...
        if (dataSets?.obj == 'contact') {
            ...
            if (this.isAgentPortal) {
                this.navigateInAgentPortal('contactDetails', {
                    'c__obj': state.c__obj,
                    'c__action': state.c__action,
                    'c__id': state.c__id
                });
            } else {
            ...
            if (state.c__action == 'TERMINATE') {
                ...
                console.log('state.c__id', state.c__id);
                if (this.isAgentPortal) {
                    this.navigateInAgentPortal('terminatePolicy', {
                        'c__id': state.c__id,
                        'c__action': state.c__action,
                        'c__obj': state.c__obj
                    });
    ...
    navigateInAgentPortal = (hashName, params) => {
        this.dispatchEvent(new CustomEvent('navigate', {
            detail: { page: hashName, params: params }
        }));
    }
