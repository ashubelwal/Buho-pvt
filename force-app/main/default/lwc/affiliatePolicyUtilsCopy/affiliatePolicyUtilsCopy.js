...
export default class AffiliatePolicyUtilsCopy extends LightningElement {
    @track isShowToast = false;
    @track toastVariant;
    @track toastHeading;
    @track agentFee ;
    @track isDisabled = true;
    @track termAgreement = {'Terms_of_Cancellation_Confirmed__c': true,
    ...
    @track booleanVar = {
        showMapfre: false, showChubb: false, showQualitas: false, isAutomobile: false, motorcyclePolicy: false, showCardPayment:true, cardPayment: false, cashPayment: false,
        ...
        isportalAccessHide: false, portalAccessNo: true, checkPortalAcccess:false,isUserDeactivated: false,disableEmail:false, emailService: true, isQuoteLoaded: false
    }
    ...
    @track trackVar = {
        ...
        Value__c: '', Year__c: '', Coverage__c: false, Territory__c: '', Vehicle_sub_type__c: '', Email: '', Phone: '', Vin__c: '', Registered_Country__c: '', Registered_State__c: '', Registered_Plate__c: '',
        paymentZip: '', paymentCity: '', paymentState: '', paymentCountry: '', paymentStreet: '', vehicleDob: '', lienHolderCountrycmbx: '', companyCountrycmbx: ''
    }

    policyTypeOptions = [
        {'label':'Automobile-Van-Minivan','value':'Automobile-Van-Minivan'},
        {'label':'RV','value':'RV'},
    ...
    get isAutomobile() {
        return this.customerData.Policy_Type__c == 'Automobile-Van-Minivan';
    }
    ...
    handleInputChange = (event) => { 
        ...
        if (event.target?.name == 'Policy_Type__c') {
            ...
            if (event.target.value == 'Automobile-Van-Minivan') {
            ...
            } else if (event.target.value == 'Motorcycle/Street Legal ATV') {
                this.booleanVar.isNorthbound = false;
                this.booleanVar.isPolicyAutomobile = false;
                ...
                this.booleanVar.isContactType = false;
                this.customerData = { ...this.customerData, ['Coverage__c']: "Complete" };
                this.trackVar = {...this.trackVar,['Vehicle_sub_type__c']:'Motor Home / RV'};
                this.customerData = { ...this.customerData, ['Vehicle_sub_type__c']: "Motor Home / RV" };
            ...
            if (event.target?.value == 'Automobile-Van-Minivan' || event.target?.value == 'RV') {
        ...
        if (event.target.name == 'Term__c') {
            ...
            if (event.target.value == 'Annual(One Year)') {
            ...
            } else if (event.target.value == 'Semi-Annual(Half a Year)') {
                this.onGroup(event.target.name, event.target.value);
    ...
    fetchQuoteData() {
        ...
        let data = {
            ...
            'Vehicle_Type__c': this.customerData?.Policy_Type__c == 'Automobile-Van-Minivan' ? 'Car/Truck/Auto' : this.customerData?.Policy_Type__c,
        ...
        console.log('Inside data check',JSON.stringify(this.customerData));
        console.log('Inside data check',JSON.stringify(data));
        console.log('Exist qupote deta',JSON.stringify(this.existQuoteDetails));
        ...
        console.log('Inside return fetched quote data');
    ...
    //     saveLeadDetails({ 'leadJson': leadJSON, 'quoteJSON': quoteJSON, 'vehicleJSON': vechileJSON, 'towedlistJSON': towedJSON , 'affiliateCode':this.affiliateCode })
    //         .then((result) => {
    //             if (result) {
    ...
    async saveLeadDetailsintoApex(leadJSON, quoteJSON, vechileJSON, towedJSON) {
        ...
        console.log('vechileJSON', vechileJSON);
        console.log('towedJSON', towedJSON);
        console.log('affiliateCode', this.affiliateCode);
        
        try {
            if (!this.affiliateCode) {
                throw new Error('Affiliate code is missing');
            }
    ...
    validateBeforeQuoteGenerationDrivers() {
        ...
        for (let eachDriver of this.addedDriver) {
            if (eachDriver.Driver_Type__c == 'Owner & Driver' || eachDriver.Driver_Type__c == 'Owner') {
    ...
    getVehiclesOptions(vehicleId){
        if(vehicleId){
            ...
            let dropdownOptions = [];
            if(this.customerData.Policy_Type__c == 'Automobile-Van-Minivan'){
                dropdownOptions = [
            ...
            getVehicleData ({'vehicleId' : vehicleId}).then((data) => {
                ...
                vehicles.map((item) => {
                    ...
                    console.log('valueExistsas',valueExists);
                    if(valueExists){
    ...
    changeDriverOption = async(event) =>{
        ...
        this.driversData.map((driver, index) => {
            console.log('Inside handleEditDriver Added Driver Map ');
