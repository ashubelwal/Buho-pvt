...
export default class Nc_quotePage extends LightningElement {
    ...
    handleOpenEngDriveLink(event) {
        ...
        let allQuoteData = this.allQuote;
        for (const company in allQuoteData) {
            if (company === quoteId) {
                const engURL = allQuoteData[company].TermAndConditionENG;
                if (this.isDebug) console.log('Eng URL', engURL);
                if (engURL) {
    ...
    handleOpenSpnDriveLink(event) {
        ...
        for (const company in allQuoteData) {
            if (company === quoteId) {
                const spnURL = allQuoteData[company].TermAndConditionSPN;
                if (this.isDebug) console.log('SPN URL', spnURL);
                if (spnURL) {
                    window.open(spnURL, '_blank');
                } else {
                    if (this.isDebug) console.log('No spanish URL exist');
                }
    ...
    async handleBuyBtn(event) {
        ...
        if (this.isDebug) console.log('handleBuyBtn All Quote: ', this.allQuote);

        let allQuoteData = this.allQuote;
        for (const company in allQuoteData) {
            if (company === quoteId) {
                this.selectedQuote = { ...allQuoteData[company], Vendor: company };
                break;
            }
        }
        ...
        this.dispatchEvent(new CustomEvent('selectquote', { detail: res }));


    }


    //For download and send email buttom
    async handleEmailandDownloadQuote(event) {
        const direction = event.currentTarget.dataset.direction; // safer than event.target
        const quoteId = event.currentTarget.dataset.buyId;
        let isEmailSent = event.currentTarget.dataset.sendEmail;
        if (this.isDebug) console.log('handleBuyBtn quoteId : ', quoteId);
        if (this.isDebug) console.log('handleBuyBtn All Quote: ', this.allQuote);
    ...
    handleLiabilityOnly(isChecked) {
        ...
        if (vehicleOptionIndex === -1) {
            if (this.isDebug) console.error('vehicleDetails object not found in payload');
            return;
        }

        if (this.isDebug) console.log('Before Update:', JSON.stringify(this.payload[vehicleOptionIndex], null, 2));
        ...
        if (!updatedPayload[vehicleOptionIndex].vehicleDetails) {
            if (this.isDebug) console.error('Vehicle Option is undefined');
            return;
        }

        // Update Liability__c safely
        updatedPayload[vehicleOptionIndex].vehicleDetails.Coverage__c = isChecked == true ? 'Liability' : 'Complete';

        this.dispatchPayloadUpdate('vehicleDetails', {
            Coverage__c: isChecked == true ? 'Liability' : 'Complete'
        });
    ...
    connectedCallback() {
        ...
        if (Array.isArray(this.payload)) {
            ...
            console.log('coverage@####??', vehicleObj.vehicleDetails.Coverage__c);
    ...
    updateTabs() {
        try {
            ...
            for (const company in data) {
                const liability = parseFloat(data[company]?.Liability) || 0;
                ...
                if (liability > 0 && total > 0) {
                    totals.push({
                        id: company,
                        label: company,
                        Gold: data[company]?.Is_Gold__c === "true",
                        Max: data[company]?.Is_Max__c === "true",
                        Platinum: data[company]?.Is_Platinum__c === "true",
                        TermAndConditionENG: data[company]?.TermAndConditionENG || '',
                        TermAndConditionSPN: data[company]?.TermAndConditionSPN || '',
                        showMedical,
            ...
            // 🔹 fallback: si no hay ningún tab válido, meter uno por defecto
            if (totals.length === 0) {
                totals.push({
                    id: 'Mapfre',
                    label: 'Mapfre',
            ...
            this.tabs = totals.map(company => {
                return {
                    ...company,
                    coverageList: company.coverageList.flatMap(coverage => Object.values(coverage).filter(Boolean)) // 🔹 evitar undefined
                };
            });

            // Orden fijo de compañías
            const companyOrderMap = {
                'Mapfre': 1,
                'Qualitas': 2,
                'Chubb': 3,
            ...
            if (!hasMapfre) {
                ...
                this.activeTab = this.activeTab?.trim() === "" ? 'Mapfre' : this.activeTab;
            }
    ...
    @api async getData() {
        ...
        try {
            ...
            // Merge updatedData into the local temp object
            temp['quotePage'] = {
                ...this.selectedQuote,
                ...updatedData
            };
        ...
        if (existingIndex !== -1) {
            // Replace existing quotePage data
            this.payload = [
    ...
    async createDataForQuoteSave() {
        ...
        const dataMapped = {
            ...
            Net_Premium__c: (parseFloat(selectedQuote?.Total) - parseFloat(selectedQuote?.IVA)) - parseFloat(selectedQuote?.TotalSurcharges),
            ...
            Quote_Value__c: (parseFloat(selectedQuote?.Total)),
            ...
            Agent_Fee__c: 0,
            Old_Net_Premium__c: (parseFloat(selectedQuote?.Total)),
