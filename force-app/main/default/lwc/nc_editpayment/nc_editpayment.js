...
export default class Nc_editpayment extends NavigationMixin(LightningElement) {
    label = {
    ...
    async connectedCallback() {
        ...
        if (this.editpolicydata != null && this.editpolicydata != undefined) {
            ...
            this.addressOfbussines = this.editpolicydata?.vehicleData?.Is_the_vehicle_registered_to_a_business__c;
    ...
    copyAddress = (event) => {
        ...
        const isBusinessRegistered = this.editpolicydata?.vehicleData?.Is_the_vehicle_registered_to_a_business__c;
        const businessAddress = this.editpolicydata?.vehicleData?.BusinessAddress__c;

        // Only company-related fields from vehicleData
        const companyInfo = {
            Company_Name__c: this.editpolicydata?.vehicleData?.Company_Name__c,
            Company_Address__c: this.editpolicydata?.vehicleData?.Company_Address__c,
            Company_Country__c: this.editpolicydata?.vehicleData?.Company_Country__c,
            Company_State__c: this.editpolicydata?.vehicleData?.Company_State__c,
            Company_City__c: this.editpolicydata?.vehicleData?.Company_City__c,
            Company_Zip__c: this.editpolicydata?.vehicleData?.Company_Zip__c,
            Company_Phone__c: this.editpolicydata?.vehicleData?.Company_Phone__c
        };

        this.bussinessAddress = isBusinessRegistered ? businessAddress : '';
        this.compnayAddress = companyInfo;

        console.log('--businessAddress--', this.bussinessAddress);
        console.log('--companyAddress--', this.compnayAddress);
        ...
        if (checked) {
            if (this.bussinessAddress) {
                this.paymentDetails = {
                    ...
                    name: this.compnayAddress?.Company_Name__c || ''
                };
            } else {
                this.paymentDetails = {
                    ...this.paymentDetails,
                    address1: this.compnayAddress.Company_Address__c || '',
    ...
    handlePayment = async () => {
        ...
        if (allValid || this.selectedCardId != '') {
            ...
            if (this.isEditPolicy == 'Yes') {
                console.log('Here in createCloneQUotes');
        ...
        try {
            ...
            if (createProfileTrans.status == 'success') {
                ...
                if (cloneNewPlocy.status == 'success') {
                    this.spinner = false;
                    const recordId = cloneNewPlocy.data
    ...
    async createAdditionalTransaction() {
        ...
        try {
            const data = await createTransactionWithPredefinedAmount({ 'newCreditCard': JSON.stringify(this.paymentDetails), 'transactionDetails': JSON.stringify(transactionDetails), 'amount': this.refundAmount, 'policyId': this.editpolicydata.policyData.Id, "email": '' });
    ...
    refundTransactions = async () => {
        ...
        console.log('Inside refund Transaction before', this.refundAmount);
