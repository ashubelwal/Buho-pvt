    ...
    async handleProceedButton() {
        if (this.trackVar?.Registered_Country__cmbx == 'Other') {
            if (this.trackVar?.Registered_Country__c.toLowerCase().includes('mexico')) {
                ...
                else { this.showToastEvent('Mexican vehicles (Mexican License Plates) are not eligible for tourist automobile insurance.', 'Contact a domestic Mexican agent or go directly to a Mexican insurance company to secure insurance on a Mexican-registered vehicle.', 'error'); }
                return;
            }
        }
