...
export default class AgentDashboard extends NavigationMixin(LightningElement) {
    ...
    async connectedCallback() {
        ...
        if (this.cmpSource == 'customSource' || this.cmpSource == 'comm') {
                    ...
                    .then((result) => {
                        ...
                        if (this.searchResults.policyData.length != 0) {
                            ...
                            this.searchResults.policyData.map(policy => {
                                ...
                                if ((policy.Status_picklist__c == 'Updated' || policy.Status_picklist__c == 'Terminated' || policy.Status_picklist__c == 'Expired') && policy.Policy_Type_picklist__c != 'Northbound') {
            ...
            if (state.c__action == 'VIEWPOLICY') {
                ...
                validateandGenerateQuotePDF({ 'policyId': state.c__id })
                    .then((result) => {
                        ...
                        if (result) {
                            window.open(('/customer/apex/' + result + '?id=' + state.c__id), '_blank');
                        } else {
