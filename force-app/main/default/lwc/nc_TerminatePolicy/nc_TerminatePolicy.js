...
export default class Nc_TerminatePolicy extends NavigationMixin(LightningElement) {
    label = {
    ...
    get brokerFeeAndAgentFeeSum() {
        return Number(this.refundObj.AgentBrokerageFee) + Number(this.refundObj.BrokerPolicyFee);
    }
    ...
    fetchRefundAmount = async () => {
        try {
            if (this.policyType === 'Watercraft' || this.policType === 'Northbound') return;
            ...
            console.log('Refund return object', this.refundObj);
    ...
    async cancleTerminate() {
        ...
        if (this.cmpSource == 'comm') {
            location.replace(`${window.location.origin}/customer/${viewurl}`);
        }
        else {
        ...
        if (this.cmpSource == 'comm') {
            ...
            if (terminatePolicy.status == 'success') {
                ...
                this[NavigationMixin.Navigate]({
                    type: 'standard__recordPage',
                    attributes: {
                        // url: `${window.location.origin}/customer/${viewurl}`
                        recordId: this.recordId,
