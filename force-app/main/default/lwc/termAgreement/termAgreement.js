...
export default class TermAgreement extends LightningElement {
     label = {
    ...
    async connectedCallback() {
        ...
        if (this.leaddata != undefined && !this.communityUser) {
            this.policyType = this.leaddata?.policyType;
        } else {
    ...
    get isShowingOrgDataPurchaseAndUse() {
        return (this.policyType == 'RV' || this.policyType == 'Automobile' || this.policyType == 'Motorcycle/Street Legal ATV' || this.policyType == 'Watercraft') && this.legalTermRecord;
    }
    get options() {
        return [
            { label: 'New', value: 'new' },
    ...
    fetchData = async () => {
        if (this.leaddata?.Id != undefined && !this.communityUser) {
            ...
            try {
                ...
                const legalTerm = await retrieveLegalTermDetails({ 'policyType': this.policyType, 'underwriter': quoteRecord.Underwriter__c });
                console.log('---legalTerm----', legalTerm);
                this.legalTermRecord = legalTerm;
                const data = await fetchDataFromLead({ "LeadId": this.leaddata?.Id, "screenName": "Terms_Alert__c" });
