...
import createNewEditPolicyFromCash from '@salesforce/apex/AffiliatePolicyEditRenewUtils.createNewEditPolicyFromCash';
import createNewEditPolicyFromCashWithAmount from '@salesforce/apex/AffiliatePolicyEditRenewUtils.createNewEditPolicyFromCashWithAmount';
import createNewEditPolicy from '@salesforce/apex/AffiliatePolicyEditRenewUtils.createNewEditPolicy';
...
import validateandGenerateQuotePDF from '@salesforce/apex/AffiliatePolicyEditRenewUtils.validateandGenerateQuotePDF';
...
import getAffiliateUser from '@salesforce/apex/AffiliatePolicyEditRenewUtils.getAffiliateUser';
import { NavigationMixin } from 'lightning/navigation';
import { CurrentPageReference } from 'lightning/navigation';
import getTimeZone from '@salesforce/apex/Mex_NewLeadProcess.getTimeZone';
import getCurrentPolicyId from '@salesforce/apex/AffiliatePolicyEditRenewUtils.getCurrentPolicyId';
import validateSession from '@salesforce/apex/AffiliatePortalController.validateSession'


export default class AffiliatePolicyEditRenew extends NavigationMixin(LightningElement) {
    // @track OldAgentFee;
    // @track OldQuote;
    // @api
    // policyId;
    ...
    get liabilityOption() {
        console.log(' this is the policy data :: ',this.policyData);
        //console.log('this.policyData.Policy_Type_picklist__c :: ', this.policyData.Policy_Type_picklist__c);
    ...
    get isPaymentNeeded() {
        ...
        console.log('Payment price', this.paymentPrice);
    ...
    get lienholderStateOption() {
        if (this.quoteData && this.quoteData?.Lienholder_Country__c != undefined) {
            if (this.quoteData && this.quoteData.Lienholder_Country__c == 'Mexico') {
                return this.mexicoStateList;
            }
            else if (this.quoteData && this.quoteData.Lienholder_Country__c == 'United States') {
                return this.unitedStatesList;
            }
            else if (this.quoteData && this.quoteData.Lienholder_Country__c == 'Canada') {
                return this.canadaStateList;
            }
        } else {
            if (this.vehicleData && this.vehicleData.Lienholder_Country__c == 'Mexico') {
                return this.mexicoStateList;
            }
            else if (this.vehicleData && this.vehicleData.Lienholder_Country__c == 'United States') {
                return this.unitedStatesList;
            }
            else if (this.vehicleData && this.vehicleData.Lienholder_Country__c == 'Canada') {
                return this.canadaStateList;
            }
        }
    }
    ...
    async connectedCallback() {
        ...
        await fetchPolicyDetails({ policyId: this.policyId })
            .then(async (result) => {
                console.log('getting contact::',result);
                const data = JSON.parse(JSON.stringify(result));
                this.contactId = data?.Contact__c;
                this.currentPolicyId = data?.Id;
    ...
    getStateParameters(currentPageReference) {
        ...
        if (currentPageReference) {
            console.log('currentPageReferance :: ',currentPageReference);
            ...
            console.log('This policy Id', this.policyId);
            console.log('This action mode', this.actionmode);
    ...
    wiringpolicyData({ data, error }) {
        if (data) {
            console.log('fetched ploicy:: ',data);
            this.policyData = JSON.parse(JSON.stringify(data));
            console.log(this.policyData);
            //console.log('this is the option for liabiality:: ',this.liabilityOption());
            if (this.actionmode == 'edit') {
            ...
            this.agencyContactId = this.policyData?.Agency__c;
            this.agentContactId = this.policyData?.Agent__c;
            this.contactId = this.policyData?.Contact__c
        } else {
            console.log('Error in fetched policy', error);
        }
    };

    @wire(getAffiliateUser, { agencyId: '$agencyContactId', agentId: '$agentContactId' })
    gettingAffiliateUser({ data, error }) {
        if (data) {
            console.log('Getting user info 56');
            const userinfo = JSON.parse(JSON.stringify(data));
            this.affiliateUserInfo = JSON.parse(userinfo);
            ...
            console.log('Agent fee in wire',this.agentFee);
            console.log('Affiliate policy fee',this.agentfeeinquote);

        } else {
            console.log('Error on the AffiliateUser 56', error);
            this.agentFee = 0;
    ...
    isInputValid = () => {
        ...
        console.log('in Validation :: ',inputFields);
    ...
    handleUpdateDriver(event) {
        ...
        for (let each of this.addedDriver) {
            if (each.license_number__c === this.selectedDriverLicense) {
                ...
                console.log(this.newDriver.isOwner);
    ...
    handleLienHolderAddress(event) {
        ...
        if ((this.policyData && this.policyData.Policy_Type_picklist__c == 'Northbound') && (fetchaddress.Country == 'United States' || fetchaddress.Country == 'Canada')) {
            ...
            if (fetchaddress.Country == 'United States' || fetchaddress.Country == 'Canada' || fetchaddress.Country == 'Mexico') {
                ...
                this.vehicleData.lienHolderCountrycmbx = fetchaddress.Country;
                this.vehicleData.Lienholder_Country__c = fetchaddress.Country;
                this.quoteData.lienHolderCountrycmbx = fetchaddress.Country;
                this.quoteData = { ...this.quoteData, ['Lienholder_Country__c']: fetchaddress.Country };
            } else {
                this.otherCountrylienholder = true;
                this.vehicleData.lienHolderCountrycmbx = 'Other';
                this.vehicleData = { ...this.vehicleData, ['Lienholder_Country__c']: fetchaddress.Country };
                this.quoteData.lienHolderCountrycmbx = 'Other';
                this.quoteData = { ...this.quoteData, ['Lienholder_Country__c']: fetchaddress.Country };
            }
        }

        this.vehicleData.Lienholder_State__c = fetchaddress.State;
        this.vehicleData.Lienholder_City__c = fetchaddress.City;
        this.vehicleData.Lienholder_Postal_Code__c = fetchaddress.PostalCode;
        this.vehicleData.Lienholder_Street__c = fetchaddress.Address;


        this.quoteData = { ...this.quoteData, ['Lienholder_State__c']: this.vehicleData.Lienholder_State__c };
        this.quoteData = { ...this.quoteData, ['Lienholder_City__c']: this.vehicleData.Lienholder_City__c };
        this.quoteData = { ...this.quoteData, ['Lienholder_Postal_Code__c']: this.vehicleData.Lienholder_Postal_Code__c };
        this.quoteData = { ...this.quoteData, ['Lienholder_Street__c']: this.vehicleData.Lienholder_Street__c };
    }
    ...
    handleInputChange = (event) => {
        try {
            ...
            else if (event.target.name == 'Territory_picklist__c') {
                this.quoteData = { ...this.quoteData, ['Territory_picklist__c']: event.target.value };
                        ...
                        .then((result) => {
                            ...
                            if (result) {
                                ...
                                createNewEditPolicy({
                                    ...
                                    'oldPolicyId': this.currentPolicyId,
                                    ...
                                    .then(async (result) => {
                                        ...
                                        getPolicyData({ 'policyId': result.data })
                                            .then((res) => {
                                                if (res === 'Success') {
                                                    console.log('Pdf gen111')
                                                }
                                                else {
                                                    console.log('Pdf ge11111n', res);
                                                }
                                            })
                                            .catch((err1) => {
                                                console.log('Error in generated pdf', err1);
                                            });
                                        // emailServices({'policyId':result.data,'emailService':this.booleanVar.emailService})
                                        // .then((nresult)=>{
                                        //     console.log(nresult);
                                        // })
                                        // .catch((nerr)=>{
                                    ...
                                    .catch((error) => {
                                        console.log('Error while creating policy1', error);
                                        if (this.cmpSource == 'comm') {
                    ...
                    createNewEditPolicyFromCash({
                        ...
                        .then(async (result) => {
                            ...
                            getPolicyData({ 'policyId': result.data })
                                .then((res) => {
                                    if (res === 'Success') {
                                        console.log('Pdf gen2222')
                                    }
                                    else {
                                        console.log('Pdf gen2222', res);
                                    }
                                });
                            emailServices({ 'policyId': result.data, 'emailService': this.booleanVar.emailService })
                                .then((nresult) => {
                                    console.log('Success in email services', nresult);
                                })
                                .catch((nerr) => {
                                    console.log('error in email services', nerr);
                                });
                            // }
                            this.booleanVar.isLoading = false;
                            this.proceedTofinalStep(result.data);
    ...
    generatePolicyPDF(event) {
        this.booleanVar.isLoading = true;
        console.log('PolicyId renewd590', this.renewdPolicyId);
        if (this.actionmode == 'renew') {
            validateandGenerateQuotePDF({ 'policyId': this.renewdPolicyId })
                .then((result) => {
                    this.booleanVar.isLoading = false;
                    console.log('In generatePolicyPdf', result);
                    if (result) {
            ...
            getCurrentPolicyId({ 'quoteData': JSON.stringify(this.editNewProcessData.quoteData) })
                .then((policyresult) => {
                    console.log('PolicyId result', this.policyresult);
                    this.renewdPolicyId = JSON.parse(JSON.stringify(policyresult));
                    console.log('PolicyId renewd589', this.renewdPolicyId);

                    validateandGenerateQuotePDF({ 'policyId': this.renewdPolicyId })
                        .then((result) => {
                            this.booleanVar.isLoading = false;
                            console.log('In generatePolicyPdf2222', result);
                            if (result) {
                               // window.open(('/apex/' + result + '?id=' + this.renewdPolicyId), '_blank');
                            } else {
                                if (this.cmpSource == 'comm') {
                ...
                .catch((error) => {
                    console.log('Some error occured in pdf section');
                    console.log(error);
                });
        }
    }
    ...
    async updatePolicy(event) {
        ...
        console.log('undating drivers ' + this.editNewProcessData.DriverData.length);
        console.log('undating vehicles ' + this.editNewProcessData.vehicleData.length);
        console.log('Quote data in update policy method', this.editNewProcessData.quoteData);
        debugger;
        ...
        console.log('Data after apex call updatepolicy details:', data);
        if (data.status == 'success') {
            ...
            getUpdatedPolicyData({ 'quoteData': JSON.stringify(this.editNewProcessData.quoteData) })
                .then((res) => {
                    if (res === 'Success') {
                        console.log('Pdf generated33333');
                    }
                    else {
                ...
                .then(async (result) => {
                    ...
                    if (email === false) {
                        getPolicyData({ 'policyId': result.data })
                            .then((res) => {
                                if (res === 'Success') {
                                    console.log('Pdf gen444')
                                }
                                else {
                                    console.log('Pdf gen4444', res);
                                }
                            });
                    }
                    this.booleanVar.isLoading = false;
    ...
    async createAdditionalTransaction(newQuoteId) {
        ...
        console.log("CA log refundamount : " + this.paymentPrice);
        try {
            const data = await createTransactionWithPredefinedAmount({ 'newCreditCard': JSON.stringify(this.paymentDetails), 'transactionDetails': JSON.stringify(transactionDetails), 'amount': this.paymentPrice.toFixed(2), 'policyId': this.currentPolicyId, "email": '' });
                ...
                if (cloneNewPlocy.status == 'success') {
                    ...
                    if (email === false) {
                        getPolicyData({ 'policyId': cloneNewPlocy.data })
                            .then((res) => {
                                if (res === 'Success') {
                                    console.log('Pdf gen555')
                                }
                                else {
                                    console.log('Pdf gen555', res);
                                }
                            });
                    }
                    this.booleanVar.isLoading = false;
    ...
    refundTransactions = async (newQuoteId) => {
        ...
        try {
            const actionData = await refundTransaction({ 'newCreditCard': JSON.stringify(this.paymentDetails), 'transactionDetails': JSON.stringify(transactionDetails), 'refundAmount': amountToRefund, 'policyId': this.currentPolicyId });
            if (actionData.status == 'success') {
                ...
                if (this.policyData.Policy_Type_picklist__c == 'Watercraft') {
                    ...
                    createCloneNewPolicy = await createNewEditPolicy({ 'vehicleData': JSON.stringify(this.vehicleData), 'DriverData': JSON.stringify(this.addedDriver), 'towedUnitData': JSON.stringify(this.addedTowed), 'quoteId': newQuoteId, 'oldPolicyId': this.currentPolicyId, 'isRenewal': false });
                }
                ...
                if (cloneNewPlocy.status == 'success') {
                    ...
                    if (email === false) {
                        ...
                        getPolicyData({ 'policyId': cloneNewPlocy.data })
                            .then((res) => {
                                if (res === 'Success') {
                                    console.log('Pdf gen666')
                                }
                                else {
                                    console.log('Pdf gen666', res);
                                }
                            });
                    }
                    this.booleanVar.isLoading = false;
    ...
    updateLienHolderDetails(event) {
        ...
        console.log('EDIT VehicleData FROM LINEHOLDER EVENT::: ', this.vehicleData);
        // } else {
        //     this.quoteData = {
        //         ...this.quoteData,
        //         ['Is_Lienholder__c']: true,
        ...
        //     console.log('RENEW QuoteData FROM LINEHOLDER EVENT::: ', this.quoteData);
        // }
