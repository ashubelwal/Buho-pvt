...
import AffiliatePolicyUtilsCopy from 'c/affiliatePolicyUtilsCopy';

import type__c from '@salesforce/schema/Transaction__ChangeEvent.type__c';
export default class AffiliateQuickPolicy extends AffiliatePolicyUtilsCopy {
                ...
                .then((result) => {
                    ...
                    if (result && result.length > 0) {
                        ...
                        if (this.existLeadDetails?.ConvertedContactId != undefined) { 
                            ...
                            if(this.cmpSource == 'comm'){
                                this.showToastmethod('error','This is already a contact to an agency.','Already Contact');
                
                            }else{
                                this.showToastEvent('Already Contact', 'This is already a contact to an agency.', 'error');
                            }
    ...
    async setExistingLeadData() {
        ...
        if (this.existLeadDetails?.Policy_Type__c == 'Automobile-Van-Minivan' || this.existLeadDetails?.Policy_Type__c == 'RV') {
        ...
        if (this.existQuoteDetails?.Term__c == 'Annual(One Year)' || this.existQuoteDetails?.Term__c == 'Annual') {
            this.booleanVar.isEndDateDisabled = true;
            this.booleanVar.isAnnual = true;
            this.booleanVar.isDaily = false;
            this.booleanVar.isSemiAnnual = false;
            this.booleanVar.isShowingEndTerm = false;
        } else if (this.existQuoteDetails?.Term__c == 'Semi-Annual(Half a Year)' || this.existQuoteDetails?.Term__c == 'Semi-Annual') {
            this.booleanVar.isEndDateDisabled = true;
        ...
        if (this.existVehicleDetails?.Lienholder_name__c !== '' && this.existVehicleDetails?.Lienholder_name__c !== undefined) {
            ...
            if (this.existVehicleDetails?.Lienholder_Country__c != '' && this.existVehicleDetails?.Lienholder_Country__c != undefined && (this.existVehicleDetails?.Lienholder_Country__c == 'United States' || this.existVehicleDetails?.Lienholder_Country__c == 'Mexico' || this.existVehicleDetails?.Lienholder_Country__c == 'Canada' || this.existVehicleDetails?.Lienholder_Country__c == 'Other')) {
                this.otherCountrylienholder = false;
                this.trackVar.lienHolderCountrycmbx = this.existVehicleDetails.Lienholder_Country__c;
            } else {
            ...
            if(this.cmpSource == 'comm'){
               ...
               if(this.existLeadDetails?.ConvertedContactId != undefined){
                   ...
                   return;
                    ...
                    location.replace(`${currentUrl}/partnercomm/quick-quote?c__contactId=${recordsId}&c__actionmode=resumeLead`);
                }
            }else{
                if(this.existLeadDetails?.ConvertedContactId != undefined){
    ...
    handleLienHolderAddress(event) {
        ...
        if(this.customerData.Policy_Type__c == 'Northbound' && (fetchaddress.Country == 'United States' || fetchaddress.Country == 'Canada')){
            ...
            if(fetchaddress.Country == 'United States' || fetchaddress.Country == 'Canada' || fetchaddress.Country == 'Mexico'){
                ...
                this.customerData = { ...this.customerData, ['Lienholder_Country__c']: this.trackVar.lienHolderCountrycmbx };
            } else {
                this.otherCountrylienholder = true;
                this.trackVar.Lienholder_Country__c = fetchaddress.Country;
                this.trackVar.lienHolderCountrycmbx = 'Other';
                this.customerData = { ...this.customerData, ['Lienholder_Country__c']: this.trackVar.Lienholder_Country__c };
            }
        ...
        this.customerData = { ...this.customerData, ['Lienholder_State__c']: this.trackVar.Lienholder_State__c };
        this.customerData = { ...this.customerData, ['Lienholder_City__c']: this.trackVar.Lienholder_City__c };
        this.customerData = { ...this.customerData, ['Lienholder_Postal_Code__c']: this.trackVar.Lienholder_Postal_Code__c };
        this.customerData = { ...this.customerData, ['Lienholder_Street__c']: this.trackVar.Lienholder_Street__c };
    }
    ...
    fetchPicklist(objectName, fieldApi) {
        ...
        getPicklistValuesFromApex({ "objectApiName": objectName, "fieldApiName": fieldApi }).then((result) => {
            ...
            getResultValue.map((item) => {
                ...
                } else if (this.policyType && (this.policyType.toLowerCase() == 'Automobile-Van-Minivan'
    ...
    getDependentPicklistValues(objectName, controllingField, dependentField) {
        getDependentMapWithTranslations({ "objectApiName": objectName, "contrfieldApiName": controllingField, "depfieldApiName": dependentField }).then((result) => {
            ...
            if (vehicleType == 'Automobile-Van-Minivan') {
    ...
    async handleDateChange(event) {
        ...
        if (name == 'Start_Date_for_Coverage__c') {
            ...
            let endDayForCoverage = startDayForCoverage;
            if (this.quoteDetails.Term__c === 'Annual(One Year)' || this.quoteDetails.Term__c === 'Annual') {
                endDayForCoverage.setDate(endDayForCoverage.getDate() + 365);
            } else if (this.quoteDetails.Term__c == 'Semi-Annual(Half a Year)' || this.quoteDetails.Term__c === 'Semi-Annual') {
                endDayForCoverage.setDate(endDayForCoverage.getDate() + 180);
            } else {
                endDayForCoverage.setDate(endDayForCoverage.getDate() + 1);
            }
    ...
    //     if (this.booleanVar.isNorthbound === false) {
    //         selectedQuote = this.allQuoteDeatils[event.target.name];
    //     }
        
    //     if (this.booleanVar.isNorthbound === true) {
    //         selectedQuote = this.allQuoteDeatils;
    //     }
    ...
    //     if (generateQuote.Policy_Type_picklist__c != 'Motorcycle/Street Legal ATV') {
    ...
    async handlePurchaseClick(event) {
        ...
        //     generateQuote = { ...generateQuote, ['Medical__c']: '' };
        // }
        //generateQuote['QuotePdfJson__c'] = jsonField;
        console.log('generateQuote--->', JSON.stringify(generateQuote));
        if (this.actionType === 'newPolicyFromContact') {
            await purchaseSaveDetails({ 'quotes': JSON.stringify(generateQuote), leadId: this.contactId, 'PolicyFromLead': false })
                .then((result) => {
            ...
            if(this.isEmailSend){
                ...
                if (sendEmailOfQuoteDetail.status == 'success') {
                    ...
                    const evt = new ShowToastEvent({
                        ...
                        title: 'Success!',
                        message: 'Email Is Sccessfully Sent',
                        variant: 'success',
                    });
                    ...
                    const evt = new ShowToastEvent({
                        //message: this.label.EmailhasbeenSent,
                        title: 'Error!',
                        message: 'Make sure to fill the email first',
                        variant: 'error',
                    });
                    this.dispatchEvent(evt);
                    this.generateLogs();
    ...
    async handleProceedButton() {
        if (this.trackVar?.Registered_Country__cmbx == 'Other') {
            if (this.trackVar?.Registered_Country__c.toLowerCase().includes('mexico')) {
                if (this.cmpSource == 'comm') {
                    this.showToastmethod('error', 'Mexican vehicles (Mexican License Plates) are not eligible for tourist automobile insurance.','Contact a domestic Mexican agent or go directly to a Mexican insurance company to secure insurance on a Mexican-registered vehicle.' );
                } else {
        ...
        if (this.addedDriver != '' && status == true) {
            if (this.validateBeforeQuoteGenerationDrivers()) {
                ...
                this.booleanVar.isLoading = false;
                if(this.cmpSource == 'comm'){
                    this.showToastmethod('error', 'For procceding further,No owner found.','Driver details are not correct!');
    
                }else{
                    this.showToastEvent('Driver details are not correct!', 'For procceding further,No owner found', 'error');
                }
            }
        } else {
            ...
            if(towedstatus == true && this.addedDriver != ''){
                if(this.cmpSource == 'comm'){
                    this.showToastmethod('error', 'Click on the pencil icon to fill in additional information related to towed unit.','Kindly provide the required details (Click the Edit button) for the tow unit(s).');
                }else{
                    this.showToastEvent('Kindly provide the required details (Click the Edit button) for the tow unit(s).', 'Click on the pencil icon to fill in additional information related to towed unit.', 'error');
                }
            }else{
                if(this.cmpSource == 'comm'){
                    this.showToastmethod('error', 'Please fill out all the required fields.','Details are not correct!');
                }else{
                    this.showToastEvent('Details are not correct!', 'Please fill out all the required fields.', 'error');
                }
            }
        }
    }
   ...
   async handlMakePayment() {
        if (this.actionType === 'newPolicyFromContact') {
            await updateContactDestinationDetails({
                ...
                console.log('Error while saving destination address through updateContactDestination',error);
            })
    ...
    handleStartTimeValidation(event){
        ...
        console.log('Date from apex', todaycheck);
              ...
              if (
               ...
               localDate.getDate() === todaycheck.getDate() && this.cmpSource == 'comm' && currentterm == 'Daily') {                   
    ...
    //         // PRIMERA PARTE: Guardar Lead/Contact Data
    //         if (this.actionType === 'newPolicyFromContact') {
    //             data = await updateContactDetails({
    //                 'Istowing': this.customerData?.Is_towing__c == 'Yes' ? true : false,
    //                 'contactId': this.contactId,
    //                 'termOptions': '',
    //                 'driverDetails': '',
    //                 'termsAlert': '',
    //                 'towedlistJSON': JSON.stringify(this.addedTowed),
    //                 'WatercraftDetails': this.allQuote?.Policy_Type_picklist == 'Watercraft' ? '' : '',
    //                 'vehicledetails': this.allQuote?.Policy_Type_picklist != 'Watercraft' ? JSON.stringify(this.fetchVehicleData()) : '',
    //                 'quoteDetails': JSON.stringify(this.fetchQuoteData())
    //             });
    ...
    //             );
    //             console.log('Lead save result:', data);
    //         }

    //         // Verificar que el guardado fue exitoso antes de continuar
    //         if (data !== 'success' && !data) {
    //             throw new Error('Failed to save lead/contact data');
    //         }

    //         // SEGUNDA PARTE: Configurar Quote Data basado en Northbound/Southbound
    //         if (this.booleanVar.isNorthbound === false) {
    //             this.allQuote = {
    //                 Policy_Type_picklist: quoteDetails?.Policy_Type__c,
    //                 Vehicle_Type: 'Automobile-Van-Minivan',
    ...
    //             let driverAge = new Date().getFullYear() - parseInt(quoteDetails.Dob__c);
    ...
    //         // TERCERA PARTE: Validaciones de Valor del Vehículo
    //         const vehicleValue = parseInt(this.allQuote.Vehicle_Value, 10);
    ...
    //             if (quoteDetails.Policy_Type_picklist == 'Automobile') {
    //                 this.booleanVar.isAutomobile = true;
    //             } else {
    //                 this.booleanVar.isAutomobile = false;
    //             }
    ...
    //         // QUINTA PARTE: Finalizar y mostrar resultados
    //         const additionalFields = {};
    //         for (const key in quoteDetails) {
    //             if (!this.allQuote.hasOwnProperty(key)) {
    //                 additionalFields[key] = quoteDetails[key];
    ...
    //         const formattedData = this.convertFinalQuoteToArray(finalQuote);
    //         this.payload = formattedData;
    //         console.log('Formatted quote data:', this.payload);
    ...
    //     } catch (error) {
    //         console.error('Error in generateQuickQuote:', error);
    //         this.booleanVar.isLoading = false;
            
    //         if(this.cmpSource == 'comm'){
    //             this.showToastmethod('error', 'An error occurred: ' + error.message, 'Error');
    //         }else{
    //             this.showToastEvent('Error', 'An error occurred: ' + error.message, 'error');
    //         }
