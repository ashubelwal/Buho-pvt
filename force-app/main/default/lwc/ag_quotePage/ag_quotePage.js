...
export default class Nc_quotePage extends LightningElement {
    ...
    @api
     ...
     try{
        ...
        console.log('Vehicle Value: ',cleanValue);
        
        if (!isNaN(cleanValue) && cleanValue > 200000) {
            allowedVendors = []; 
            this.dispatchEvent(
            new CustomEvent('policyrestriction', {
                detail: {
                    message: 'You cannot purchase policy for this vehicle value please click on previous button.',
                    type: 'error'
                },
                bubbles: true,
                composed: true
            ...
            if (allowedVendors.includes(company) && parseFloat(data[company].Liability) > 0 && parseFloat(data[company].Total) > 0) {
                totals.push({
                    ...
                    total: (
                        ...
                        parseFloat(data[company].AgentFee) +
                        Number(this.agentuserfee)
                    ).toFixed(2),
                    coverageList: [
                        {
    ...
    async createDataForQuoteSave() {
        ...
        const dataMapped = {
            ...
            Net_Premium__c: (parseFloat(selectedQuote?.Total) - parseFloat(selectedQuote?.IVA) - parseFloat(selectedQuote?.BrokerFee) - parseFloat(selectedQuote?.TotalSurcharges) - parseFloat(selectedQuote?.AgentFee) + parseFloat(Number(this.agentuserfee))).toFixed(2),
            ...
            Quote_Value__c: (parseFloat(selectedQuote?.Total) - parseFloat(selectedQuote?.AgentFee) + parseFloat(Number(this.agentuserfee))).toFixed(2),
            ...
            Agent_Fee__c: this.agentuserfee,
            Old_Net_Premium__c: (parseFloat(selectedQuote?.Total) - parseFloat(selectedQuote?.AgentFee) + parseFloat(Number(this.agentuserfee))).toFixed(2),
